...
(async () => {
  ...
  const usdcCoin = tx.moveCall({
    ...
    arguments: [tx.object(USDC_TREASURY), tx.pure.u64(20n * 1_000_000n)],
  });

  const suiCoin = tx.moveCall({
    target: `${TEST_COIN_PACKAGE}::sui::mint`,
    arguments: [tx.object(SUI_TREASURY), tx.pure.u64(10n * 1_000_000_000n)],
  });

  tx.transferObjects([usdcCoin, suiCoin], keypair.toSuiAddress());
